...
contract('main', (_accounts) => {
  ...
  describe('Basic Setup Test', async () => {
    ...
    it('Total Supply is correct', async () => {
      const totalSupply = new BigNumberJS(await instance.methods.totalSupply().call());
      const expectedTotalSupply = new BigNumberJS(0.6 * (10 ** 9) * (1 * 10 ** 18));
  ...
  describe('Minter role test', async () => {
    const amount = 1000;
    let controllerBalance, transferResult, contractBalance, randomWalletBalance, prevRandomWalletBalance;    
    const tokenHolderAddress = web3.eth.accounts[3];
    const randomWalletAddress = web3.eth.accounts[5];
    const controllerAddress = web3.eth.accounts[4];
    const minterAddress1 = web3.eth.accounts[6];
    const minterAddress2 = web3.eth.accounts[7];
    it('Controller can add a minter address', async() => {
      const addMinterResult = await instance.methods.addMinter(minterAddress1).send({ from: controllerAddress});            
    });
    it('Non controller cannot add a minter address', async() => {
      try {
        expect(await instance.methods.addMinter(minterAddress2).send({ from: randomWalletAddress})).to.be.rejectedWith(Error);            
      } catch (error) {
        //
      }      
    });
    ...
    it('Mint works by minter', async () => {      
      ...
      await instance.methods.mint(randomWalletAddress, web3.toWei(amount)).send({ from: minterAddress1 })
      randomWalletBalance = web3.fromWei(await instance.methods.balanceOf(randomWalletAddress).call());            
      assert.equal(String(randomWalletBalance), String(Number(amount)+Number(prevRandomWalletBalance)));
    });

    it('Minter can be removed by controller', async () => {
      await instance.methods.removeMinter(minterAddress1).send({ from: controllerAddress })      
    });

    it('Minter cannot be removed by non-controller', async () => {
      try {
        expect(await instance.methods.removeMinter(minterAddress1).send({ from: randomWalletAddress })).to.be.rejectedWith(Error);
      } catch (error) {
        //
      }      
    });

    it('Removed minter can no longer mint', async () => {
      try {
        expect(await instance.methods.mint(randomWalletAddress, web3.toWei(amount)).send({ from: minterAddress1 })).to.be.rejectedWith(Error);
      } catch (error) {
        //
      }      
    });
