...
contract PropsRewardEntities is Initializable, Ownable {
    ...
    */
    ...
    struct Application {
        ...
        uint256 timeAdded;                       // Daily timestamp of when the application was added
        uint256 timeRemoved;                     // Daily timestamp of when the application was removed
    }
    ...
    struct Validator {
        ...
        uint256 initializedState;                // A way to check if there's something in the map and whether already added to list
        uint256 timeAdded;                       // Daily timestamp of when the validator was added
        uint256 timeRemoved;                     // Daily timestamp of when the validator was removed
    }
    ...
    */
     ...
     modifier onlyParticipatingActiveValidators(address _address, uint256 _dailyTimestamp) {
         require(
             validators[_address].status == ValidatorStatus.Active && _dailyTimestamp > validators[_address].timeAdded,
             "Must be an active validator"
             );
         _;
     }
    ...
    */
    function approveApplication(
        ...
        onlyOwner
        onlyExistingApplications(_id)
        onlyValidDailyTimestamp(_timestamp)
        returns (bool)
    {
        ...
        applications[_id].timeAdded = _timestamp;
        applications[_id].status = ApplicationStatus.Active;
        emit ApplicationUpdated
        (
            _id, applications[_id].name,
            applications[_id].rewardsAddress,
            applications[_id].sidechainAddress,
            applications[_id].status
        );
        return true;
    }
    ...
    */
    ...
    {
        applications[_id].status = ApplicationStatus.Inactive;
        applications[_id].timeRemoved = _timestamp;
        emit ApplicationUpdated
        (
            _id,
            applications[_id].name,
            applications[_id].rewardsAddress,
            applications[_id].sidechainAddress,
            applications[_id].status
        );
    ...
    */
        ...
        returns (bool)
    {
        ...
        (
            _id,
            validators[_id].name,
            validators[_id].rewardsAddress,
            validators[_id].sidechainAddress,
            validators[_id].status
        );
        return true;
    }
    ...
    */
        ...
        returns (bool)
    {
        validators[_id].status = ValidatorStatus.Inactive;
        emit ValidatorUpdated
        (_id, validators[_id].name, validators[_id].rewardsAddress, validators[_id].sidechainAddress, validators[_id].status);
        return true;
    }
}