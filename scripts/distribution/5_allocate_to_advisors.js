...
module.exports = async() => {
  ...
  csv
    .fromPath(`csv/advisors/${network}.csv`)
    .on("data", async(row) => {
        ...
        try {
          ...
          if (web3.isAddress(allocation.investor) && allocation.tokenVolume.greaterThan(0)) {
            ...
            if (tx.transaction.logs.find(log => log.event === 'Execution')) {
              const log = `Advisor Allocation for ${allocation.investor} on line #${count}, buying ${allocation.tokenVolume}.`
              console.log(`✔︎ ${log}`);
              config
                .transactions
