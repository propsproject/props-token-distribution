...
if (typeof (newAdmin) === 'undefined') {
  console.log('Must supply newAdmin');
  process.exit(0);
}
...
async function main() {
  ...
  const upgradeToEncoded = zos.encodeCall('changeAdmin', ['address'], [newAdmin]);
  console.log(`upgradeToEncoded=${upgradeToEncoded}`);
  console.log(`Issuing changeAdmin to ${newAdmin} via multisig wallet ${multisigWalletForPropsTokenProxy}`);
