...
module.exports = async() => {
  ...
  csv
    .fromPath(`csv/foundation/${network}.csv`)
    .on("data", async(row) => {
      ...
      if (count > 0) {
        ...
        try {
          ...
          if (web3.isAddress(allocation.investor) && allocation.tokenVolume.greaterThan(0)) {
            ...
            console.log(tx.transaction.logs);

            if (tx.transaction.logs.find(log => log.event === 'Execution')) {

              const log = `Foundation Allocation for ${allocation.investor} on line #${count}, buying ${allocation.tokenVolume}.`
              console.log(`✔︎ ${log}`);
              config
                .transactions
