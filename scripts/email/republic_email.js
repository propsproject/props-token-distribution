...
function prepareBody(recipient) {
  ...
  body += '<br><br>';
  body += 'Thank you for supporting Props. Your Props Tokens have been delivered to the wallet address you provided. You\'ll find a detailed summary of your Props holdings and the way you can use them below:';
  body += '<br>';
  body += '<ul>';
  ...
  if (parseInt(recipient.investedDiscount)>0 && recipient.investedDiscount!='16.67') {
  ...
  if (parseFloat(recipient.investedAmount)>0) {
    if (recipient.investedDiscount=='16.67') {
      body += `${'<li>' + 'Initial Token allocation: '}${republicTokens(recipient.totalTokens).toLocaleString('en')}</li>`;
      body += '<li>' + 'Your bonus: 20%</li>';
      body += `${'<li>' + 'Token allocation after bonus: '}${initialTokens(recipient.totalTokens).toLocaleString('en')}</li>`;
    } else {
      body += `${'<li>' + 'Initial Token allocation: '}${initialTokens(recipient.totalTokens).toLocaleString('en')}</li>`;
    }
    body += '</ul>';
    body += 'As you remember, we chose to reward our early investors with an additional 10% bonus in March 2018. ';
    body += 'Given that bonus, your token allocation has increased:';
    body += '<br>';
    body += '<ul>';
    body += `${'<li>' + 'Additional bonus Tokens: '}${roundDown(parseFloat(recipient.totalTokens) - initialTokens(recipient.totalTokens), 2).toLocaleString('en')}</li>`;
  }
...
function republicTokens(tokens) {
  return roundDown(parseFloat(tokens, 1.2, 10) / 1.1 / 1.2, 2);
}
